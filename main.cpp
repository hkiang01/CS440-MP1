...
void astar_3(cell** maze, maze_props props)
{
	...
	while(!frontier.empty())
	{
		...
		frontier.pop();
		if(DEBUG) {
			cout << "Expanding " << current_cell->x << "," << current_cell->y ;
		}
		
		expansions++;
		
		//Check if we reached one of the goals just now
		for(unsigned int i=0; i<goals_list.size(); ++i) {
			if(current_cell == goals_list.at(i))
			{
				//Premanently remove this goal node from being a "goal"
				goals_list.erase(goals_list.begin()+i);
				current_cell->goal_order=goal_counter;
				cout<<"     Reached a goal, giving ("<<current_cell->x<<","
					<<current_cell->y<<") goal #"<<goal_counter<<endl;
				goal_counter++;
				//i--;
				
				
				//Update PriQueue by re-pushing everything?
		...
		//Update all total_costs since we've committed to a node just now, and going back would mean something else in astar3
		//Recalculate all manhattan distances, now excluding this goal we just acomplished
		set_manhattan_distances_3(maze, props, goals_list, current_cell);
		
		
		// Push adjacent nodes onto frontier
		int cx = current_cell->x, cy = current_cell->y;
...
int main(int argc, char* argv[])
{
	...
	switch (mode) {
		...
		case 'A' :
			set_manhattan_distances_3(maze, props, props.goals, NULL);
			astar_3(maze, props);
			break;
